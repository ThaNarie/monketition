    ...
    <ProfileUserContext.Provider value={user}>
      <div className="profile-page pt-5">
        ...
        <div className="recent-matches row row-cols-1 row-cols-md-3 row-cols-lg-4 g-4 mb-5">
          <div className="col">
            <StreakCard
              streakInfo={user.participants[0]}
              sport={getMockSport({ type: 'table-tennis' })}
            />
          </div>
          <div className="col">
            <StreakCard streakInfo={user.participants[0]} sport={getMockSport({ type: 'pool' })} />
          </div>
          <div className="col">
            <StreakCard
              streakInfo={user.participants[0]}
              sport={getMockSport({ type: 'foosball' })}
            />
          </div>
        </div>
        <RecentMatches matches={user.participants.flatMap(({ matches }) => matches)} />
        <Section>
          <h3>Awards</h3>
